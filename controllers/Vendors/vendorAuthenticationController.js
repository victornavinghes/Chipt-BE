...
const { userAuthenticationResponses } = require('../../utils/responseObjects.js');
...
exports.projectName_Vendor_Account_First_Time_Login = CatchAsync(async (req, res, next) => {
    ...
    const vendor = await Vendors.findOne({ primaryEmail: email.toLowerCase()})
    ...
    let message = `Dear Vendor,\n\nYour store account verification OTP is ${OTP}. Use it before ${new Date(expireTime).toString()} otherwise it will gone a expire.\n\nThanks,\nChipt`
    try {
        await sendEmail({
...
exports.projectName_Vendor_Account_OTP_Verification = CatchAsync(async (req, res, next) => {
    ...
    const vendor = await Vendors.findById({ _id: vendorID }).select("+accountActive +oneTimePassword")
    if (!vendor) {
    ...
    if (vendor.userOTP.timeToExpire <= Date.now()) {
        ...
        vendor.userOTP.timeToExpire = Date(Date.now() - (24 * 0 + 60 * 0 + 60 * 0 + 5900))
        vendor.userOTP.OTPVerifed = false
...
exports.projectName_Vendor_Account_Resend_OTP = CatchAsync(async (req, res, next) => {
    ...
    let emailSuccess = false
    let expireTime = Date.now() + 960000
    let message = `Dear Vendor,\n\nVerification OTP for your account registration is ${OTP}. Use it before ${new Date(expireTime).toString()} otherwise it will gone be expire.\n\nThanks,\nChipt`
    try {
        await sendEmail({
            email: vendor.primaryEmail,
            subject: 'Account OTP',
...
// 04) ✅ VENDOR: Sign in
exports.projectName_Vendor_Account_Sign_In = CatchAsync(async (req, res, next) => {
    ...
    const vendorExist = await Vendors.findOne({ primaryEmail: email.toLowerCase() }).select("+accountActive +password +oneTimePassword");
    ...
    } else if (!vendorExist || !await vendorExist.correctPassword(password, vendorExist.password)) {
        return next(new ErrorHandler('Invalid Credential', 401))
    }


    // d) Fetching User data
    const vendor = await Vendors.findById({ _id: vendorExist._id }).select("+alleyAuth userName email OTPVerified +accountActive +accountVerified")
...
// 05) ✅ VENDOR: Sign out
exports.projectName_Vendor_Account_Sign_Out = CatchAsync(async (req, res, next) => {
...
exports.projectName_Vendor_Account_Password_Update = CatchAsync(async (req, res, next) => {
    ...
    const vendor = await Vendors.findById({ _id: vendorID })
        .select('username +password +oneTimePassword +accountActive +accountVerified +name');
    ...
    if (vendor.oneTimePassword && !vendor.password) {
        ispasswordMatch = req.body.oldpassword === vendor.oneTimePassword ? true : false;
    } else if (!vendor.oneTimePassword && vendor.password) {
        ispasswordMatch = await vendor.correctPassword(req.body.oldpassword, vendor.password);
    }
    if (!ispasswordMatch) {
...
exports.projectName_Vendor_Account_Password_Forgot_OTP = CatchAsync(async (req, res, next) => {
    ...
    const vendor = await Vendors.findOne({ primaryEmail: req.body.email.toLowerCase() })
    if (!vendor) {
    ...
    let emailSend = false
    let expireTime = Date.now() + 960000;
    const message = `Dear Vendor,\n\nOTP for you account password recovery is:- ${otp}, Use it before ${new Date(expireTime).toString()} otherwise it will expire. \n\n If you have not request this email then please ignore this!\n\nThanks\nChipt`;
    await sendEmail({
...
exports.projectName_Vendor_Account_Reset_OTP_Verification = CatchAsync(async (req, res, next) => {
...
exports.projectName_Vendor_Account_Password_Reset_After_OTP_Verified = CatchAsync(async (req, res, next) => {
    ...
    if (!vendor.forgotOTP.OTPVerifed) {
        return next(new ErrorHandler(`OTP is not verified, Please try again after some time!`, 400))
    }
    if (req.body.newPassword !== req.body.confirmPassword) {
        return next(new ErrorHandler("Passwords doesn't match.", 404));
    }
...
exports.projectName_Vendor_Account_Password_Forgot_Email = CatchAsync(async (req, res, next) => {
    ...
    const vendor = await Vendors.findOne({ primaryEmail: req.body.email.toLowerCase() })
    if (!vendor) {
        return next(new ErrorHandler(`No such vendor exist with this email!`, 404));
    }

    // c) Get ResetPasswordToken
    const resetToken = await vendor.getResetPasswordToken();
    await vendor.save({ validateBeforeSave: false });
    const resetPasswordUrl = `${req.protocol}://${req.get("host")}/api/v1/vendor/password/reset/${resetToken}`;
...
exports.projectName_Vendor_Account_Password_Reset = CatchAsync(async (req, res, next) => {
...
exports.projectName_Vendor_Account_Informations = CatchAsync(async (req, res, next) => {
    ...
    const vendor = await Vendors.findById({ _id: vendorID })
        .select("+accountActive +accountVerified +registrationID +name username primaryEmail +secondaryEmail countryCode +primaryContactNumber +secondaryContactNumber +dateOfRegistration plotnumber address city state country zipCode profilePicture location")
    ...
    // c) Sending response
    userAuthenticationResponses.userProfileInformationResponse(res, 200, 'vendor', vendor)
})

// 13) ✅ VENDOR: Account information update
exports.projectName_Vendor_Account_Information_Update = CatchAsync(async (req, res, next) => {

    // a) Fetching vendor data and checking if it exists
    const vendorCheck = await Vendors.findById({ _id: req.user.id })
        .select("primaryEmail +secondaryEmail +primaryContactNumber +secondaryContactNumber")
    if (!vendorCheck) {
        return next(new ErrorHandler(`Vendor doesn't exist with Email address provided`, 404))
    }
    ...
    if (req.body.email) {
        if (req.body.email.toLowerCase() === vendorCheck.primaryEmail) {
            ...
            const emCheck = await utilsMiddleware.userEmailExistanceCheck(req.body.email.toLowerCase());
            if (emCheck) {
    ...
    if (req.body.contact) {
        if(req.body.contact === vendorCheck.primaryContactNumber){}
            ...
            const cnCheck = await utilsMiddleware.userContactExistanceCheck(req.body.countryCode, req.body.contact)
            if (cnCheck) {
                return next(new ErrorHandler(`Contact number already in use by other user, Try new one.`, 409))
            }
        }
    }
...
// 14) ✅ VENDOR: Store location update
exports.projectName_Vendor_Store_Location_Updated = CatchAsync(async (req, res, next) => {

    // a) Fetching user ID from 
    const vendorId = req.user.id;
    const location = req.body.location;
    if (!location) return next(new ErrorHandler(`Please provide location`, 400))
...
// 15) ✅ VENDOR: Profile image upload
exports.projectName_Vendor_Account_Profile_Image_Upload = CatchAsync(async (req, res, next) => {
    ...
    const myCloud = await cloudinary.v2.uploader.upload_stream({ folder: `vendors/${vendor._id}/profilePicture` },
...
// 16) ✅ VENDOR: Profile image delete
exports.projectName_Vendor_Account_Profile_Image_Delete = CatchAsync(async (req, res, next) => {
...
exports.projectName_Vendor_Account_Store_Images_Upload = CatchAsync(async (req, res, next) => {
    // a) Variable declaration and fetching customerID 
    let resizedImage;
    let vendorID = req.user.id;
    ...
    // d) Compressing image 
    await utilsMiddleware.utilsUploadMultipleImages(uploadImages)
        .then((data) => {
            resizedImage = data
        })
    ...
    for (let i = 0; i < resizedImage.length; i++) {
        const myCloud = await cloudinary.v2.uploader.upload_stream({ folder: `vendors/${vendor._id}/gallery` },
            async function (err, image) {
                let temp = {
                    public_id: image.public_id,
                    url: image.url,
                ...
                if (uploadLenCheck === resizedImage.length) {
                    await vendor.save()
                    userAuthenticationResponses.userGelleryImagesUploadResponse(res, 200)
                }
            }
        );
        // g) Saving data using stream
...
exports.projectName_Vendor_Account_Store_Image_Delete = CatchAsync(async (req, res, next) => {
    // a) Variable declaration and fetching customerID 
    let vendorID = req.user.id;
...
exports.projectName_Vendor_Account_Notification = CatchAsync( async( req, res, next)=>{

    const vedorID = req.user.id
    
    const vendorNotification = await InAppNotification.find({toVendor: vedorID, messageRead: false})
