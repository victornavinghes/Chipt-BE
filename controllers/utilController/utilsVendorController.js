...
exports.projectName_Utils_Vendor_Account_All_Store_Images = CatchAsync(async (req, res, next) => {

    // a) Fetching vendor id
    if (req.params.id) {
        vendorID = req.params.id
    } else {
        const vendor = await Vendors.findById({ _id: req.user.id }).select("+primaryRole")
...
exports.projectName_Utils_All_Vendors_Store_Cups_Stocks = CatchAsync(async (req, res, next) => {

    // a) Fetching stock information
    const vendorStock = await StoreCupsStock.find()
        .populate("vendor", 'name primaryEmail primaryContactNumber profilePicture')
        .sort({ createdAt: -1 })
    if (!vendorStock || vendorStock.length === 0) return next(new ErrorHandler(`No data found`, 200))
...
exports.projectName_Utils_All_Vendors_Cup_Stock_Requests = CatchAsync(async (req, res, next) => {
    ...
    const stockRequest = await CupStockRequest.find()
        ...
        .populate('cupsRequested.cupID', 'cupModelUniqueId cupType cupSize cupCapacity cupPrice currency')
        .sort({ createdAt: -1 })
    ...
    // c) Sending response
    storeStockResponse.storeStockRequestsResponse(res, 200, stockRequest, true, 0)
})

// 04) ✅ UTILS: All vendors pending stock requests
exports.projectName_Utils_All_Vendor_Pending_Cup_Stock_Requests = CatchAsync(async (req, res, next) => {
    ...
    // c) Sending response
    storeStockResponse.storeStockRequestsResponse(res, 200, stockRequest, true, 1)
})

// 05) ✅ UTILS: All vendors rejected stock requests
exports.projectName_Utils_All_Vendor_Rejected_Cup_Stock_Requests = CatchAsync(async (req, res, next) => {
    ...
    // c) Sending response
    storeStockResponse.storeStockRequestsResponse(res, 200, stockRequest, true, 2)
})

// 06) ✅ UTILS: All vendors accepted stock requests
exports.projectName_Utils_All_Vendor_Accepted_Cup_Stock_Requests = CatchAsync(async (req, res, next) => {
    ...
    // c) Sending response
    storeStockResponse.storeStockRequestsResponse(res, 200, stockRequest, true, 3)
})

// 07) ✅ UTILS: Single vendor cups stock in store 
exports.projectName_Utils_Single_Vendor_Store_Cup_Stock = CatchAsync(async (req, res, next) => {
    ...
    const vendor = await Vendors.findOne({
        ...
        .select("+name primaryEmail username profilePicture +accountActive +storeCupsStock")
...
exports.projectName_Utils_Single_Vendor_Cup_Stock_Requests = CatchAsync(async (req, res, next) => {
...
exports.projectName_Utils_Single_Vendor_Pending_Cup_Stock_Requests = CatchAsync(async (req, res, next) => {
...
exports.projectName_Utils_Single_Vendor_Rejected_Cup_Stock_Requests = CatchAsync(async (req, res, next) => {
    ...
    if (!stockRequest || stockRequest.length === 0) {
        return next(new ErrorHandler(`Single vendor stock requests not found.`, 200))
    }
...
exports.projectName_Utils_Single_Vendor_Accepted_Cup_Stock_Requests = CatchAsync(async (req, res, next) => {
...
exports.projectName_Utils_Vendor_Single_Cup_Stock_Request_Information = CatchAsync(async (req, res, next) => {
    ...
    if (!stockRequest) {
        return next(new ErrorHandler(`Vendor single stock request not found`, 200))
    }

    // c) Sending response
    storeStockResponse.storeStockRequestsResponse(res, 200, stockRequest, false, 0)
})

// 13) ✅ UTILS: Dashboard Information for single vendor
exports.projectName_Utils_Vendor_Show_All_Information = CatchAsync(async (req, res, next) => {
    ...
    // Cup Data
    const tempCups = await StoreCupsStock.findOne({ vendor: req.user.id })
    ...
    // Orders data
    const orders = await CupOrders.find({ vendor: req.user.id });

    const cupRequests = await CupStockRequest.find({ vendor: req.user.id });

    // Data assignment
    vendorData.customers = !customers || customers.length === 0 ? 0 : customers.length;
