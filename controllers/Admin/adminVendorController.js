...
async function reduce_stock(cid, nmber) {
    const inventoryCup = await CupInventory.findById({ _id: cid })
        .select("+cupsAvailable")
...
async function increase_stock(requestedCupData) {
    const inventoryCup = await CupInventory.find()
        .select("+cupsAvailable numberOfCups")
    for (let i = 0; i < requestedCupData.length; i++) {
        ...
        for (let j = 0; j < inventoryCup.length; j++) {
            ...
            if (rid === cid) {
                inventoryCup[j].numberOfCups += parseInt(requestedCupData[i].numberOfCups);
            }
        }
    }
    await inventoryCup.save()
...
exports.projectName_Admin_Vendor_Account_Registration = CatchAsync(async (req, res, next) => {

    // a) Destructuring request body and checking for all data
    const { name, email, oneTimePassword, countryCode, contact, plotnumber, address, city, state, country, zipCode, location } = req.body
    if (!name || !email || !oneTimePassword || !countryCode || !contact || !plotnumber || !address || !city || !state || !country || !zipCode || !location) {
    ...
    const passwordValidation = projectValidation.passwordValidation(req.body.oneTimePassword);
    ...
    if (req.body.email) {
        ...
        if (emCheck) {
            return next(new ErrorHandler(`User with this ${req.body.email.toLowerCase()} address has been registered before`, 409))
        }
    }
    ...
    if (!emailSuccess) {
        await Vendors.findByIdAndDelete({ _id: vendor._id })
            .catch((err) => {
                return next(new ErrorHandler(`Something went wrong`, 200))
            })
        return next(new ErrorHandler(`Something went wrong while sending email`, 404))
    }

    utilsMiddleware.sendNotificationToUser('admin', 'vendor', req.user.id, vendor._id, 'Account creation', `${req.body.name.charAt(0).toUpperCase() + req.body.name.slice(1)} account has beend created.`)
...
exports.projectName_Admin_All_Vendors_List = CatchAsync(async (req, res, next) => {

    // a) Fetching all vendors
    const vendor = await Vendors.find()
        .select("name username primaryEmail +countryCode +primaryContactNumber profilePicture +accountActive +accountVerified +plotnumber +address +city +state +country +zipCode")
    if (vendor.length === 0) {
...
exports.projectName_Admin_Single_Vendor_Information = CatchAsync(async (req, res, next) => {
    ...
    const vendor = await Vendors.findById({ _id: vendorID })
        .select("+registrationID +name primaryEmail username +countryCode +primaryContactNumber profilePicture +accountActive +plotnumber +address +city +state +country +zipCode")
        .catch((err) => {
            return next(new ErrorHandler(`Something went wrong`, 200))
        })
    if (!vendor) {
...
exports.projectName_Admin_Vendor_Account_Enable_Disable = CatchAsync(async (req, res, next) => {
    ...
    if (!vendor.accountVerified) {
        return next(new ErrorHandler(`Not allowed to block as it's account is not verified`, 400))
    }
    ...
    res.status(200).json({
        success: true,
        message: `Vendor account is now ${vendor.accountActive ? 'active.' : 'inactive.'}`
    })
})

// 05) ✅ ADMIN: Active vendor accounts
exports.projectName_Admin_All_Active_Vendors_Accounts = CatchAsync(async (req, res, next) => {
...
exports.projectName_Admin_All_Inactive_Vendors_Accounts = CatchAsync(async (req, res, next) => {
...
exports.projectName_Admin_Reject_Vendor_Stock_Request = CatchAsync(async (req, res, next) => {
    ...
    if (cupRequest.approvalStatus === 'STATUS_ACCEPTED' && cupRequest.requestStatus === 'DELIVERED') {
    ...
    for (let i = 0; i < inventoryCup.length; i++) {
        const le = await CupInventory.findByIdAndUpdate({ _id: inventoryCup[i]._id }, { numberOfCups: inventoryCup[i].numberOfCups }, { new: true })
    }
    cupRequest.approvalStatus = "STATUS_REJECTED"
    cupRequest.requestStatus = "REJECTED"
    await cupRequest.save();
    utilsMiddleware.sendNotificationToUser('admin', 'vendor', req.user.id, cupRequest.vendor._id, 'Stock rejection', `Dear vendor, Your stock request (ID: ${req.params.rid}) has been rejected by Admin.`);
...
exports.projectName_Admin_Accept_Vendor_Stock_Request = CatchAsync(async (req, res, next) => {
    ...
    if (cupRequest.approvalStatus === 'STATUS_ACCEPTED' && cupRequest.requestStatus === 'DISPATCH' ||
        cupRequest.approvalStatus === 'STATUS_ACCEPTED' && cupRequest.requestStatus === 'SHIPPED'
    ) {
        return next(new ErrorHandler(`Request already been accepted.`, 200))
    }
    if (cupRequest.approvalStatus === "STATUS_REJECTED" && cupRequest.requestStatus === "REJECTED") {
    ...
    for (let i = 0; i < cupRequest.cupsRequested.length; i++) {
        reduce_stock(cupRequest.cupsRequested[i].cupID._id, cupRequest.cupsRequested[i].numberOfCups)
    }
...
exports.projectName_Admin_Confirm_Delivery_Vendor_Store_Stock = CatchAsync(async (req, res, next) => {
    ...
    if (cupRequest.approvalStatus === "STATUS_PENDING" && cupRequest.requestStatus === "PROCESSING") {
        return next(new ErrorHandler(`Not allowed to update vendor stock before request approval.`, 200))
    }
    if (cupRequest.approvalStatus === 'STATUS_ACCEPTED' && cupRequest.requestStatus === 'DISPATCH') {
        return next(new ErrorHandler(`Currently, stock can't be updated before stock approval.`, 200))
    }
    if (cupRequest.approvalStatus === "STATUS_REJECTED" && cupRequest.requestStatus === "REJECTED") {
        return next(new ErrorHandler(`This request is already been rejected.`, 200))
    }
    if (cupRequest.approvalStatus === 'STATUS_ACCEPTED' && cupRequest.requestStatus === 'DELIVERED') {
        return next(new ErrorHandler(`Vendor stock already been delivered.`, 200))
    }
