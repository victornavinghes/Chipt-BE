...
const { userAuthenticationResponses, customerResponses } = require('../../utils/responseObjects.js');
...
// 01) ✅ CUSTOMER: SIGN UP
exports.projectName_Customer_Account_Sign_Up = CatchAsync(async (req, res, next) => {
    ...
    const isContactExist = await utilsMiddleware.userContactExistanceNoCodeCheck(req.body.contactNumber)
    const isEmailExist = await utilsMiddleware.userEmailExistanceCheck(req.body.email.toLowerCase())
    if (isContactExist) {
        return next(new ErrorHandler(`User already exist with same contact.`, 400))
    }
    if (isEmailExist) {
    ...
    if (!emailSuccess) {
        ...
        return next(new ErrorHandler(`Something went wrong while sending OTP`, 200))
    } else {
        // h) Sending notification
        utilsMiddleware.sendNotificationToUser(false, 'customer', null, customer._id, 'Sign Up', `Account has been created, Please verify your account using OTP sent to your email.`)
...
// 02) ✅ CUSTOMER: OTP VERIFICATION 
exports.projectName_Customer_Account_OTP_Verification = CatchAsync(async (req, res, next) => {
    ...
    const customer = await Customers.findById({ _id: customerID }).select('+accountActive +accountVerified')
    if (!customer) {
    ...
    if (customer.userOTP.OTPVerifed) {
        ...
        if (customer.userOTP.otp === parseInt(req.body.otp)) {
            ...
            utilsMiddleware.sendNotificationToUser(false, 'customer', null, customer._id, 'Account verification', `Your account has been verified!`)
            return res.status(200).json({
                success: true,
...
// 03) ✅ CUSTOMER: RESEND OTP 
exports.projectName_Customer_Account_Resend_OTP = CatchAsync(async (req, res, next) => {

    // a) Destructuring logged in vendor ID and destructuring request body
    const customerID = req.user.id

    // b) Fetching vendor and checking if vendor exists
    const customer = await Customers.findById({ _id: customerID }).select("+accountActive +accountVerified firstname primaryEmail")
    if (!customer) {
    ...
    utilsMiddleware.sendNotificationToUser(false, 'customer', null, customer._id, 'Resend OTP', `Account verification OTP sent to your email!`)
    // g) Sending response
    res.status(200).json({
        success: true,
...
// 04) ✅ CUSTOMER: SIGN IN 
exports.projectName_Customer_Account_Sign_In = CatchAsync(async (req, res, next) => {
    ...
    const customerExist = 
        await Customers.findOne({ primaryEmail: contactNumber.toString() })
            .select("+accountActive +isBlocked +password +firstname +middlename +lastname")
        ||
        await Customers.findOne({ primaryContactNumber: parseInt(contactNumber) })
        .select("+accountActive +isBlocked +password +firstname +middlename +lastname")

    // c.1) Checking if account is active
    if (customerExist.isBlocked) {
        return next(new ErrorHandler('Dear user, your account has been suspended by admin!', 403))
    }

    // c.2) Checking password are same or not
    if (!customerExist || !await customerExist.correctPassword(password, customerExist.password)) {
        return next(new ErrorHandler('Invalid Credential', 401))
    }

    // d) Fetching User data
    const customer = await Customers.findById({ _id: customerExist._id }).select("+accountActive +alleyAuth userName email OTPVerified")
...
// 05) ✅ CUSTOMER: SIGN OUT 
exports.projectName_Customer_Account_Sign_Out = CatchAsync(async (req, res, next) => {
...
// 06) ✅ CUSTOMER: PASSWORD UPDATE
exports.projectName_Customer_Account_Password_Update = CatchAsync(async (req, res, next) => {

    // a) Extracting customer id from logged in vendor
    const customerID = req.user.id
    if (!req.body.oldPassword || !req.body.newPassword || !req.body.confirmPassword) return next(new ErrorHandler(`Please provide passwords`, 400))
    ...
    if(!customer){
        return res.status(200).json({
            success: false,
            message: 'Something went wrong while changing password, Please try again after sometime!'
        })
    }
    ...
    const ispasswordMatch = await customer.correctPassword(req.body.oldPassword, customer.password);
    if (!ispasswordMatch) {
        return res.status(200).json({
            success: false,
    ...
    utilsMiddleware.sendNotificationToUser(false, 'customer', null, customer._id, 'Password update', `Account password has been updated.`);
    authToken.userSendToken(res, 200, customer, 'passwordupdate', 'customer')
})

// 07) ✅ CUSTOMER: FORGOT PASSWORD USING OTP
exports.projectName_Customer_Account_Password_Forgot_OTP = CatchAsync(async (req, res, next) => {

    // a) Destructuring request body and checking if email provided
    const { email } = req.body

    if (!email) {
        return next(new ErrorHandler(`Please provide your registered Email`, 401));
    }

    // b) Fetching vendor and checking if vendor exist
    const customer = await Customers.findOne({ primaryEmail: req.body.email.toLowerCase() })
    ...
    let emailSend = false
    const message = `Dear customer,\n\nGreeting of the day.\n\nYour account reset verification OTP is ${otp}, Please use this otp before ${Date.now() + 960000} otherwise it will expire. \n\n If you have not request this email then please ignore this!\n\nThanks\nChipt`;
    await sendEmail({
...
// 08) ✅ CUSTOMER: RESET OTP VERIFICATION
exports.projectName_Customer_Account_Reset_OTP_Verification = CatchAsync(async (req, res, next) => {
...
exports.projectName_Customer_Account_Password_Reset_After_OTP_Verified = CatchAsync(async (req, res, next) => {
    ...
    // e) Sending response
    utilsMiddleware.sendNotificationToUser(false, 'customer', null, customer._id, 'Sign Up', `Account password has been reset.`);
    userAuthenticationResponses.userPasswordResetResponse(res, 200, 'customer');
})

// 10) ✅ CUSTOMER: FORGOT PASSWORD USING EMAIL
exports.projectName_Customer_Account_Password_Forgot_Email = CatchAsync(async (req, res, next) => {
    ...
    if (!customer) {
        return next(new ErrorHandler(`No such vendor exist with this email!`, 200));
    }
    ...
    const resetPasswordUrl = `${req.protocol}://${req.get("host")}/api/v1/customer/authentication/password/reset/email/${resetToken}`;

    // d) Sending email to vendor and if it fail sending error response
    let emailSend = false
    const message = `Dear ${customer.firstname ? customer.firstname : 'Customer'},\n\nGreetings of the day,\n\nYour account password reset url is:- \n\n ${resetPasswordUrl}, \n\nIf you have not request this email then please ignore it.\n\nThanks\nChipt`;
    await sendEmail({
        email: customer.primaryEmail,
        subject: 'Chipt Account Password Reset',
        message,
...
exports.projectName_Customer_Account_Password_Reset_Email = CatchAsync(async (req, res, next) => {
    ...
    const passwordValidation = projectValidation.passwordValidation(req.body.newPassword);
    if (!passwordValidation.success) return next(new ErrorHandler(`${passwordValidation.message}`, 400));
    ...
    // f) Sending response
    utilsMiddleware.sendNotificationToUser(false, 'customer', null, customer._id, 'Account recovery', `Account has been recovered successfully.`);
    userAuthenticationResponses.userPasswordResetResponse(res, 200, 'customer')
})

// 12) ✅ CUSTOMER: PROFILE INFORMATION
exports.projectName_Customer_Account_Informations = CatchAsync(async (req, res, next) => {
    ...
    const customer = await Customers.findById({ _id: customerID })
        .select("+accountActive +accountVerified username +firstname +middlename +lastname +dateOfBirth profilePicture primaryEmail +gender countryCode primaryContactNumber +plotnumber +address +city +state +country +zipCode location");
    if (!customer) {
...
exports.projectName_Customer_Account_Contact_Update = CatchAsync(async (req, res, next) => {
    ...
    if (req.body.username) {
        if (req.body.username !== customer.username) {
            ...
            const usernameCheck = await utilsMiddleware.userUsernameExistanceCheck(req.body.username);
            if (usernameCheck) {
                return res.status(200).json({
                    success: false,
    ...
    // d) Email Check
    if (req.body.email) {
        if (req.body.email.toLowerCase() !== customer.primaryEmail.toLowerCase()) {
            const emailValidation = projectValidation.emailValidation(req.body.email.toLowerCase());
            if (!emailValidation.success) {
                return next(new ErrorHandler(`${emailValidation.message}`, 400));
            }
            const emailCheck = await utilsMiddleware.userEmailExistanceCheck(req.body.email.toLowerCase());
            if (emailCheck) {
                return next(new ErrorHandler(`User with this ${req.body.email} address has been registered before`, 409))
            }
        }
    }
    ...
    if (req.body.contact && req.body.countryCode) {
        ...
        if (customer.countryCode && customer.primaryContactNumber) {
            if (req.body.countryCode !== customer.countryCode.toString() && req.body.contact.toString() !== customer.primaryContactNumber.toString()) {
                const contactCheck = await utilsMiddleware.userContactExistanceCheck(req.body.countryCode, req.body.contact);
                if (contactCheck) {
                    return next(new ErrorHandler(`User with this ${req.body.contact} contact has been registered before`, 409))
                }
            }
        } else {
            const contactCheck = await utilsMiddleware.userContactExistanceCheck(req.body.countryCode, req.body.contact);
    ...
    customer.username = req.body.username ? req.body.username.toLowerCase() : customer.username.toLowerCase();
    customer.primaryEmail = req.body.email ? req.body.email.toLowerCase() : customer.primaryEmail.toLowerCase();
    if (customer.countryCode && customer.primaryContactNumber) {
        customer.countryCode = req.body.countryCode ? req.body.countryCode : customer.countryCode
        customer.primaryContactNumber = req.body.contact ? req.body.contact : customer.primaryContactNumber
    } else {
        customer.countryCode = req.body.countryCode ? req.body.countryCode : undefined
        customer.primaryContactNumber = req.body.contact ? req.body.contact : undefined
    }
    await customer.save();

    // g) Sending response
    utilsMiddleware.sendNotificationToUser(false, 'customer', null, customer._id, 'Contact update', `Account contact information has been updated.`);
    customerResponses.customerProfileInformation(res, 200, customer);
})

// 14) ✅ CUSTOMER: Basic information update
exports.projectName_Customer_Account_Basic_Information_Update = CatchAsync(async (req, res, next) => {
    ...
    // b) Saving details
    customer.firstname = req.body.firstname ? req.body.firstname.toLowerCase() : customer.firstname ? customer.firstname.toLowerCase() : undefined;
    customer.middlename = req.body.middlename ? req.body.middlename.toLowerCase() : customer.middlename ? customer.middlename.toLowerCase() : undefined;
    customer.lastname = req.body.lastname ? req.body.lastname.toLowerCase() : customer.lastname ? customer.lastname.toLowerCase() : undefined;
    customer.dateOfBirth = req.body.dateOfBirth ? req.body.dateOfBirth : customer.dateOfBirth ? customer.dateOfBirth : undefined;
    customer.gender = req.body.gender ? req.body.gender.toLowerCase() : customer.gender ? customer.gender.toLowerCase() : undefined;
    await customer.save()

    // c) Sending response
    utilsMiddleware.sendNotificationToUser(false, 'customer', null, customer._id, 'Basic information update', `Your account baisc information has been updated.`);
    customerResponses.customerProfileInformation(res, 200, customer, false)
})

// 15) ✅ CUSTOMER: Address update
exports.projectName_Customer_Account_Address_Update = CatchAsync(async (req, res, next) => {
    ...
    // b) Saving details
    customer.plotnumber = req.body.plotnumber ? req.body.plotnumber.toLowerCase() : customer.plotnumber ? customer.plotnumber.toLowerCase() : undefined;
    customer.address = req.body.address ? req.body.address.toLowerCase() : customer.address ? customer.address.toLowerCase() : undefined;
    customer.city = req.body.city ? req.body.city.toLowerCase() : customer.city ? customer.city.toLowerCase() : undefined;
    customer.state = req.body.state ? req.body.state.toLowerCase() : customer.state ? customer.state.toLowerCase() : undefined;
    customer.city = req.body.city ? req.body.city.toLowerCase() : customer.city ? customer.city.toLowerCase() : undefined;
    customer.country = req.body.country ? req.body.country.toLowerCase() : customer.country ? customer.country.toLowerCase() : undefined;
    customer.zipCode = req.body.zipCode ? req.body.zipCode : customer.zipCode ? customer.zipCode : undefined;
    await customer.save()

    // c) Sending response
    utilsMiddleware.sendNotificationToUser(false, 'customer', null, customer._id, 'Address', `Your adddress information has been updated.`);
    customerResponses.customerProfileInformation(res, 200, customer)

})

// 16) ✅ CUSTOMER: Location Update
exports.projectName_Customer_Account_Location_Update = CatchAsync(async (req, res, next) => {
    ...
    customer.location.coordinates = req.body.location ? req.body.location : customer.location ? customer.location : null;
    await customer.save();

    // c) Sending response
    utilsMiddleware.sendNotificationToUser(false, 'customer', null, customer._id, 'Location update', `Your current location has been updated.`);
    customerResponses.customerProfileInformation(res, 200, customer)
})

// 17) ✅  CUSTOMER: PROFILE IMAGE UPLOAD
exports.projectName_Customer_Account_Profile_Image_Upload = CatchAsync(async (req, res, next) => {
    ...
    if (customer.profilePicture.public_id.toString() !== "default/user_jvowub") {
        await cloudinary.uploader.destroy(customer.profilePicture.public_id)
    }

    const myCloud = await cloudinary.v2.uploader.upload_stream({ folder: `customers/${customer._id}/profilePicture` },
    ...
    utilsMiddleware.sendNotificationToUser(false, 'customer', null, customer._id, 'Profile image updated', `Your profile picture has been updated successfully!.`);
    await streamifier.createReadStream(resizedImage.data).pipe(myCloud)
})

// 18) ✅  CUSTOMER: Profile Image Delete
exports.projectName_Customer_Account_Profile_Image_Delete = CatchAsync(async (req, res, next) => {
    ...
    // e) Sending response
    utilsMiddleware.sendNotificationToUser(false, 'customer', null, customer._id, 'Profile image delete', `Your profile picture has been deleted successfully!.`);
    res.status(200).json({
        success: true,
...
exports.projectName_Customer_Account_Profile_Image = CatchAsync(async (req, res, next) => {
    ...
    // b) Fetching vendor details
    const customer = await Customers.findById({ _id: customerID })
        .select("profilePicture username primaryContactNumber")
...
exports.projectName_Customer_Account_Notification = CatchAsync(async (req, res, next) => {
    ...
    if (queryData) {
        console.log('yes')
        const l1 = await InAppNotification.findByIdAndUpdate({ _id: queryData }, { messageRead: true })
            .catch((err) => { queryData = false });
    }
    const notificationCount = await InAppNotification.countDocuments({ toCustomer: customerID, messageRead: false });
    ...
    res.status(200).json({
        ...
        currentLength: req.query.page ? (req.query.page * resultPerPage) - (resultPerPage - allNotification.length) : allNotification.length,
        allNotification
    })
})

// 22) Fetching Account Status
exports.projectName_Customer_Account_Status = CatchAsync(async (req, res, next) => {
    const customerID = req.user.id
    const customer = await Customers.findById(customerID).select("+accountVerified");
