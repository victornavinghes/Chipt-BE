exports.userAuthenticationResponses = {
    ...
    userRegistrationResponse: function (res, statusCode, userCategory, token, user) {
        return res.status(statusCode).json({
            success: true,
            message: `Dear ${userCategory === 'admin' ? 'Admin' : userCategory === 'vendor' ? 'Vendor' : 'User'} verification OTP sent to your email address!`,
            token: token,
        })
    },

    // b) User -> Admin, Vendor, and Customer Sign Up
    userSignUpResponse: function (res, statusCode, userCategory, token, user, firstTime) {
        return res.status(statusCode).json({
            success: true,
    ...
    userLoginInResponse: function (res, statusCode, userCategory, token, user, firstTime) {
        return res.status(statusCode).json({
            success: true,
    ...
    userPasswordChangeResponse: function (res, statusCode, userCategory, token, user) {
        return res.status(statusCode).json({
            success: true,
    ...
    userProfileInformationResponse: function (res, statusCode, userCategory, user) {
        let data;
        if (userCategory === 'admin') {
            ...
            if (user.secondaryContactNumber) data.secondaryContactNumber = user.secondaryContactNumber
        ...
        return res.status(statusCode).json({
            success: true,
            message: `${userCategory === 'admin' ? 'Admin' : userCategory === 'vendor' ? 'Vendor' : 'User'} profile information!`,
            user: data
        });
    },
...
exports.adminResponses = {
    ...
    adminProfileInformationResponse: function (res, statusCode, user, msgCheck) {
        ...
        let resData = {
            success: true,
            message: msgCheck ? `Admin profile information!` : `Admin profile updated successfully!`
        }
...
exports.vendorResponses = {
    ...
    vendorStoreStockInformationResponse: function (res, statusCode, user, isList) {

        let data;
        return res.status(statusCode).json({
            success: true,
            message: `${isList ? 'Vendors stock inforamtion' : 'Vendor stock inforamtion'}`,
            vendors: data
        })
    },

    // b) Vendor stock response
    vendorStoreRequestInformationResponse: function (res, statusCode, user, isAll, isList) {

        let data;
        return res.status(statusCode).json({
            success: true,
            message: `${isList ? isAll ? 'Vendors stock requests' : 'Vendor stock requests' : 'Vendor stock request'}`,
            vendors: data
        })
    }
}
        ...
        if (isList) {
            ...
            customers = users.map((data) => {
                let temp = {
                    ...
                    primaryContactNumber: data.primaryContactNumber ? data.primaryContactNumber : null,
            ...
            customers = {
                ...
                dateOfBirth: users.dateOfBirth ? users.dateOfBirth.toString().split(' ').slice(0, 4) : null,
...
exports.cupsResponses = {
    ...
    cupsInInventoryResponse: function (res, statusCode, cups, isList, isAvailable) {
        ...
        if (isList) {
            ...
            for (let i = 0; i < cupTempData.length; i++) {
                if (i == 0) {
                    ...
                    const checkCupTypeExist = this.checkCupTypeExist(finalData, cupTempData[i].cupType);
                    if (checkCupTypeExist.isCupExist) {
                        finalData[checkCupTypeExist.indexVal].availableSize.push(cupTempData[i].availableSize[0])
                    } else {
            ...
            resData.message = isAvailable === 2 ? "All cups in inventory " : isAvailable ? `Available cups in inventory` : `Unavailable cups in inventory`
    ...
    coffeeCupHistoryResponse: (res, statusCode, cupModelDetails, cupHistry, isHistory) => {
        let cupTempData;
        let resData = {
            success: true,
            message: 'Cup details',
        ...
        let coffeeCupData = {
            ...
            cupType: cupModelDetails.cupType.charAt(0).toUpperCase() + cupModelDetails.cupType.slice(1),
            cupSize: cupModelDetails.cupSize.charAt(0).toUpperCase() + cupModelDetails.cupSize.slice(1),
        ...
        if (!isHistory) {
            ...
            let mn = cupHistry.currentCustomer.middlename ? " " + cupHistry.currentCustomer.middlename.charAt(0).toUpperCase() + cupHistry.currentCustomer.middlename.slice(1) + " " : " "
            let temp = {
                currentVendor: {
                    cvId: cupHistry.currentVendor._id,
                    name: cupHistry.currentVendor.name.charAt(0).toUpperCase() + cupHistry.currentVendor.name.slice(1),
                },
                currentCustomer: {
                    ccId: cupHistry.currentCustomer._id,
                    name: cupHistry.currentCustomer.firstname.charAt(0).toUpperCase() + cupHistry.currentCustomer.firstname.slice(1) + mn + cupHistry.currentCustomer.lastname.charAt(0).toUpperCase() + cupHistry.currentCustomer.lastname.slice(1)
                }
            }
            if (cupHistry.lastVendor) {
                const lastTemp = {
                    lvId: cupHistry.lastVendor._id,
                    name: cupHistry.lastVendor.name.charAt(0).toUpperCase() + cupHistry.lastVendor.name.slice(1)
                }
...
exports.storeStockResponse = {
        ...
        storeStock.cups.forEach((data) => {
            if (cupData.inStock.length === 0) {
                ...
                for (let i = 0; i < cupData.inStock.length; ++i) {
                    if (cupData.inStock[i].cupType.toLowerCase() === data.cupID.cupType.toLowerCase()) {
                        const temp = {
                            cupSizeID: data.cupID._id,
                            cupModelUniqueId: data.cupID.cupModelUniqueId,
                            cupSize: data.cupID.cupSize,
    ...
    storeStockRequestsResponse: function (res, statusCode, requests, isList, currentStatus) {
...
exports.scannedCupResponse = {
    ...
    cupDetailsAfterCupScan: function (res, statusCode, cupDetails, orderHistory, uniqueID, returnCondition) {
        ...
        let returnData = {
            ...
            cupType: cupDetails.cupType.charAt(0).toUpperCase() + cupDetails.cupType.slice(1),
            cupSize: cupDetails.cupSize.charAt(0).toUpperCase() + cupDetails.cupSize.slice(1),
...
exports.orderCupsResponse = {
    ...
    returnedCupsResponse: function (res, statusCode, returedCups){
        ...
        const returned_Cups = returedCups.map((data) => {
            ...
            let firstname = data.customer.firstname ? data.customer.firstname +' ' : '';
            let middlename = data.customer.middlename ? data.customer.middlename +' ' : '';
            let lastname = data.customer.lastname ? data.customer.lastname : '';
            temp.customer.name = firstname + middlename + lastname;
            let customerCountryCode = data.customer.countryCode ? "+" + data.customer.countryCode : ''; 
            let customerContact = data.customer.primaryContactNumber ? data.customer.primaryContactNumber : ''
            temp.customer.contact = customerCountryCode + customerContact;

            let vnedorCountryCode_F = data.fromVendor.countryCode ? "+" + data.fromVendor.countryCode : ''; 
            let vendorContact_F = data.fromVendor.primaryContactNumber ? data.fromVendor.primaryContactNumber : '';
            temp.fromVendor.contact = vnedorCountryCode_F + vendorContact_F;

            let vnedorCountryCode_R = data.returnedVendor.countryCode ? "+" + data.returnedVendor.countryCode : ''; 
            let vendorContact_R = data.returnedVendor.primaryContactNumber ? data.returnedVendor.primaryContactNumber : '';
